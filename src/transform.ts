...
import { codeceptjsStringifyChromeRecording } from './main.js';
...
export function runTransformsOnChromeRecording({
  ...
  return files.map(async (file) => {
    console.log(
      chalk.green(`Running CodeceptJS Chrome Recorder on ${file}\n`),
    );

    const recordingContent = readFileSync(file, 'utf-8');
    const stringifiedFile = await codeceptjsStringifyChromeRecording(
      recordingContent,
    );
